...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `argocd` with the `NAME` of your provider.
...
- binary: pulumi-resource-argocd
  ...
  ignore: []
  ldflags:
  - -X github.com/noarqerimi/argocd/tree/main/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-argocd/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── argocd_darwin_amd64_v1
│   └── pulumi-resource-argocd
├── argocd_darwin_arm64
│   └── pulumi-resource-argocd
├── argocd_linux_amd64_v1
│   └── resource-argocd
├── argocd_linux_arm64
│   └── resource-argocd
├── argocd_windows_amd64_v1
│   └── resource-argocd.exe
└── argocd_windows_arm64
    └── pulumi-resource-argocd.exe
```
